...
# for model 1
q1_sf$uni <- q1_sf$uni_q1
q1_sf$gender <- q1_sf$gender_q1
q1_sf$age_scaled <- q1_sf$age_scaled_q1

q2_sf$uni <- q2_sf$uni_q2
q2_sf$gender <- q2_sf$gender_q2
q2_sf$age_scaled <- q2_sf$age_scaled_q2

### model components
if (share_covariates) {
  # model 1: shared sampling intensity, shared fixed and random effects
  cmp_multi <- ~ -1 + 
    # shared field of sampling intensity
    Intercept_pop(1) + field_pop(geometry, model = spde) + 
    # shared individual random effect
    pid_eff(pid, model = "iid") +
    # shared fixed and random effects
    uni_eff_shared(uni, model = "iid") +          
    gender_eff_shared(gender, model = "iid") +    
    age_eff_shared(age_scaled, model = "linear") +
    # question 1
    Intercept_q1(1) + 
    ...
    field_q2(geometry, model = spde)    #W2
  
} else {
  # model 2: shared sampling intensity, separate fixed and random effects
  cmp_multi <- ~ -1 + 
    # shared field of sampling intensity
    Intercept_pop(1) + 
...
if (share_covariates) {
  ...
  lik_q1 <- bru_obs(
    "binomial",
    formula = bi_ans_q1 ~ Intercept_q1 + uni_eff_shared + gender_eff_shared + age_eff_shared + pid_eff + field_pop_copy_q1 + field_q1,
    data = q1_sf, Ntrials = 1
  )
  # likelihood 4: binomial response for Q2
  lik_q2 <- bru_obs(
    "binomial",
    formula = bi_ans_q2 ~ Intercept_q2 + uni_eff_shared + gender_eff_shared + age_eff_shared + pid_eff + field_pop_copy_q2 + field_q2,
    data = q2_sf, Ntrials = 1
  )
...
ggsave(
  filename = "output/figures/joint_predictions_Q1_Q2.pdf",   #_share_covariates
